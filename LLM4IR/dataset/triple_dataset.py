...
class TripleDatasetBase(Dataset):
    def __init__(
        self,
        data_dir,  # Root directory of the MBEIR dataset
    ):
        self.beir_data_dir = data_dir
...
class TripleTrainDataset(TripleDatasetBase):
    def __init__(
        self,
        data_dir,  
        query_data_path,
    ...
    def __getitem__(self, index):
        ...
        entry = self.query_data[index]
        query_txt = entry.get("query") or ""

        pos_cand_list = entry.get("pos_cand_list", [])
        assert len(pos_cand_list) > 0, f"Cannot find positive candidates for {entry}"

        selected_pos_cand_did = self.select_cand(pos_cand_list)
        pos_cand = self.cand_pool.get(selected_pos_cand_did)
        assert pos_cand, f"Cannot find positive candidate {selected_pos_cand_did} for {entry}"
        ...
        if self.mode == Mode.TRAIN:
            neg_cand_id_list = entry.get("neg_cand_list", [])
            if self.hard_neg_num > 0:
                assert len(neg_cand_id_list) > 0, f"Cannot find negative candidates for {entry}"
...
class TripleTestDataset(TripleDatasetBase):
    def __init__(
        self,
        data_dir,  
        query_data_path,
    ...
    def _load_query_as_dict(self, query_data_path):
        self._load_query_data(query_data_path)
        query_data_dict = {}
...
class TripleCandidatePoolDataset(TripleDatasetBase):
    def __init__(
        self,
