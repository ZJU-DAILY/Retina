...
# class SparsePairwiseFlopsLengthLoss(torch.nn.Module):
#     def __init__(self):
#         super().__init__()
#         self.ce_loss = CrossEntropyLoss()
#         self.flops_loss = FLOPS()
...
#     def forward(self, query_embeddings, doc_embeddings, query_mask, doc_mask):
#         """
#         query_embeddings: (batch_size, dim)
#         doc_embeddings: (batch_size, dim)
#         """
#         query_length = torch.sum(query_mask, dim=1).float()
#         doc_length = torch.sum(doc_mask, dim=1).float()
...
#         scale_factor = torch.sqrt(query_length.max() * doc_length.max()) * 1e-3
        
#         self.lambda_flops_query = scale_factor * self.lambda_flops_query
#         self.lambda_flops_doc = scale_factor * self.lambda_flops_doc
